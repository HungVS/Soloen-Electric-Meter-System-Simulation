...
export class DCU extends Component <any,any> {
  static displayName = DCU.name;
  id: any;
  dcuID: string;
  ...
  componentDidMount() {
    this.dragElement(document.querySelector(`#${this.dcuID}`));
  }
  componentDidUpdate() {
    ...
    if(this.props.name ==='DCU'){
     ...
     return (
      <div id={this.props.id} className="mydiv" style={{top: this.props.y, left: this.props.x}} onMouseMove = {this.handleChange}>
        ...
        <text fill="#ffffff" fontSize={120} fontFamily="Verdana" x={79} y={450}> {id} </text>
        </svg>
        <br/>
    ...
    { 
      this.setState({ [e.target.name]: e.target.value }, () => {
        if(this.props.id !== undefined) {
          ...
          if(temp.includes(this.props.id)){
            console.log(this.props.id)
            axios.post('/api/ksp',{
              targetNode : this.props.id
            }).then(response =>{
