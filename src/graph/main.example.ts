...
(() => {
    ...
    const numberVertex = 10
    const vertexlist : Vertex[] =  []
    const weight : number []= [4,3,20,1,  4,2,  2,20,  3,20,5,  5,1,  20,1,99 ,99  ,1]
    for (let i = 0 ; i < numberVertex ; i++) {
        vertexlist.push(new Vertex(i))
    }
    vertexlist[0].addVertexAdjency([vertexlist[1],vertexlist[3],vertexlist[5],vertexlist[7]])
